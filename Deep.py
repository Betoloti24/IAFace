...
clasificacion = cv2.CascadeClassifier(cv2.data.haarcascades + "haarcascade_frontalface_default.xml")
...
while True:
    ...
    # Procesamos
    faces = clasificacion.detectMultiScale(frame, 1.3, 5)
    ...
    for (x,y,w,h) in faces:
        ...
        # Informacion
        info = DeepFace.analyze(face, actions=['age', 'gender', 'race', 'emotion'], enforce_detection= False)
    ...
    #         print(f"Datos persona {cantidad}: \nEdad: {edad}\nGenero: {gen}\nEmocion: {emociones}")
    #         # Mostramos info
